...
class SRData(data.Dataset):
    def __init__(self, args, train=True, benchmark=False):
        ...
        self.scale = args.scale # 降采样尺度，default='4'
        self.idx_scale = 0 #这个大小是什么不确定
        # C:\\Users\\17581\\Desktop\\RCAN-master\\RCAN-master
        ...
        # defalut：'sep_reset'    
            ...
            if args.ext.find('reset') >= 0:
                ...
                for si, s in enumerate(self.scale):
                    for v in self.images_lr[si]: # x4图片
                        lr = imageio.imread(v)
                        name_sep = v.replace(self.ext, '.npy')
                        np.save(name_sep, lr)
            # hr里面的文件从png变成npy
            ...
            # lr里面的文件从png变成npy，但是为什么image_lr是二维数组？
            self.images_lr = [
